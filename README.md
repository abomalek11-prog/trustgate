...
| 5 | Open **Graph** (`/graph`). | The agent network (who signed what for whom) and the verifier's LangGraph-style state machine light up edge by edge as the same request is evaluated. |
...
app/ + components/   Console · Verification Trace · Decision Card · Attack Lab · Policy Editor · JSON drawer · Agent Directory · Trust Profile · Agent Graph · Architecture
...
Nowhere in the decision path — deliberately. Every node of the verifier's graph is a deterministic predicate over signed bytes; there is no model call, no embedding, no score. An LLM (or any planner) can sit *in front of* an agent and decide **what** to ask for; whatever it proposes must still be signed by the agent's key and pass the gate. A hallucinating or prompt-injected planner can propose a $2,000 purchase — the graph still ends in DENY, because authority comes from a principal's signature, not from anything a model says. If you build agents with LangGraph / AutoGen / CrewAI, TrustGate is the gate node between *decide* and *act*: `POST /api/gate` with the signed request, branch on `decision`. The `/graph` page draws both the agent network and the verifier's state machine.

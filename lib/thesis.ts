/**
 * The two-year thesis (≤300 words). Single source of truth: rendered on
 * /architecture, included in README via docs/thesis.md, and length-checked by
 * tests/thesis.test.ts.
 */
export const THESIS = `Over the next two years, agent identity will split into three layers: cryptographic identity, portable evidence, and local trust policy.
...
export const THESIS_WORD_COUNT = THESIS.split(/\s+/).filter(Boolean).length;
